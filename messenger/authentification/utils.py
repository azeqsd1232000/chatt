...
async def send_bienvenue(email,name):
    async with httpx.AsyncClient() as client:
        response = await client.post("https://serveur-node-4.onrender.com/sendemail/welcome", json={"to": email, "name": name},timeout=20)
        return response

#la fonction qui envoie le code a 5 chiffre a l'email de l'utilsateur       
async def send_otp(email, otp):
    async with httpx.AsyncClient() as client:
        response = await client.post("https://serveur-node-4.onrender.com/sendemail/otp", json={"to": email, "otp": otp})
        return response
